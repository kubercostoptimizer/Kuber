...
local function _UploadUserId(req_id, post, carrier)
  ...
  local user_client = GenericObjectPool:connection(UserServiceClient, "user-service.social-network.svc.cluster.local", 9090)
  local status, err = pcall(user_client.UploadCreatorWithUserId, user_client, req_id, tonumber(post.user_id), post.username, carrier)
  if not status then
...
local function _UploadText(req_id, post, carrier)
  ...
  local text_client = GenericObjectPool:connection(TextServiceClient, "text-service.social-network.svc.cluster.local", 9090)
...
local function _UploadMedia(req_id, post, carrier)
  ...
  local media_client = GenericObjectPool:connection(MediaServiceClient, "media-service.social-network.svc.cluster.local", 9090)
  local status, err
  if (not _StrIsEmpty(post.media_ids) and not _StrIsEmpty(post.media_types)) then
    status, err = pcall(media_client.UploadMedia, media_client,req_id, cjson.decode(post.media_types), cjson.decode(post.media_ids), carrier)
  else
    status, err = pcall(media_client.UploadMedia, media_client,req_id, {}, {}, carrier)
  end
  if not status then
    ngx.status = ngx.HTTP_INTERNAL_SERVER_ERROR
...
function _M.ComposePost()
  ...
  local span = tracer:start_span("ComposePost",{ ["references"] = { { "child_of", parent_span_context } } })
  local carrier = {}
