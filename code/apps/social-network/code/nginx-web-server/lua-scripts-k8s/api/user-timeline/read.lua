...
function _M.ReadUserTimeline()
  ...
  local span = tracer:start_span("Login", {["references"] = {{"child_of", parent_span_context}}})
  local carrier = {}
  ...
  if (timestamp + ttl < ngx.time()) then
    ...
    local client = GenericObjectPool:connection(UserTimelineServiceClient, "user-timeline-service.social-network.svc.cluster.local", 9090)
    local status, ret = pcall(client.ReadUserTimeline, client, req_id,user_id, tonumber(args.start), tonumber(args.stop), carrier)
