...
void UserTimelineHandler::WriteUserTimeline(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("WriteUserTimeline");
  ...
  if (!updated) {
    ...
    started_mongo_find = std::chrono::high_resolution_clock::now();
    updated = mongoc_collection_find_and_modify(collection, query, nullptr, update, nullptr, false, false, true, &reply, &error);
    timetaken_mongo_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;
...
void UserTimelineHandler::ReadUserTimeline(std::vector<Post> &_return, int64_t req_id, int64_t user_id, int start, int stop, const std::map<std::string, std::string> &carrier) {
  ...
  // Initialize a span
  std::map<std::string, std::string> writer_text_map;
  auto span = opentracing::Tracer::Global()->StartSpan("ReadUserTimeline");
  ...
  try {
    ...
    timetaken_mongo_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    LOG(warning) << timetaken_mongo_call;
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;
  } catch (...) {
  ...
  std::future<std::vector<Post>> post_future = std::async(
      std::launch::async, [&]() {
        ...
        std::vector<Post> _return_posts;
          ...
          try {
            post_client->ReadPosts(_return_posts, req_id, post_ids, writer_text_map);
          } catch (...) {
