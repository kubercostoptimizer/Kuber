...
TClient * ClientPool<TClient>::Pop() {
  ...
  std::unique_lock<std::mutex> cv_lock(_mtx); {
    while (_pool.size() == 0) {
      ...
      if (_curr_pool_size < _max_pool_size) {
        try {
          ...
          LOG(warning) << "creating new client";
          _curr_pool_size++;
          break;
        } catch (...) {
          cv_lock.unlock();
        ...
        bool wait_success = _cv.wait_until(cv_lock, wait_time, [this] { return _pool.size() > 0; });
        if (!wait_success) {
    ...
    if (!client){
      client = _pool.front();
      LOG(warning) << "getting old one";
      _pool.pop_front();
    }

  } // cv_lock(_mtx)
  cv_lock.unlock();
  LOG(warning) <<"pop start";
  if (client) {
    try {
      client->Connect();
    } catch (...) {
