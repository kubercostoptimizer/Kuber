...
void UserHandler::RegisterUserWithId(
  ...
  strs << timetaken-timetaken_by_external_calls;
  span->SetBaggageItem("runtime_ms", strs.str());
...
void UserHandler::RegisterUser(
  ...
  // Compose user_id
  int64_t timestamp = duration_cast<microseconds>(system_clock::now().time_since_epoch()).count() - CUSTOM_EPOCH;
...
void UserHandler::UploadCreatorWithUserId(
  ...
  LOG(warning)<<"logging time-taken: "<<timetaken-timetaken_by_external_calls;
  span->Log({{"runtime_ms",timetaken-timetaken_by_external_calls}});
  span->Finish();
}
...
int64_t UserHandler::GetUserId(
  ...
  // TextMapWriter writer(writer_text_map);
  // auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("GetUserId");
  // opentracing::Tracer::Global()->Inject(span->context(), writer);
