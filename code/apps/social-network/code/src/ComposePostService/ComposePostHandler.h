...
void ComposePostHandler::UploadCreator(int64_t req_id, const Creator &creator, const std::map<std::string, std::string> &carrier) {
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("UploadCreator");

  std::string creator_str = "{\"user_id\": " + std::to_string(creator.user_id) + ", \"username\": \"" + creator.username + "\"}";
...
void ComposePostHandler::UploadText(int64_t req_id, const std::string &text, const std::map<std::string, std::string> &carrier) {
...
void ComposePostHandler::UploadMedia(int64_t req_id, const std::vector<Media> &media, const std::map<std::string, std::string> &carrier) {
...
void ComposePostHandler::UploadUniqueId(int64_t req_id, const int64_t post_id, const PostType::type post_type, const std::map<std::string, std::string> &carrier) {
  ...
  auto hset_reply_0 = redis_client->hset(std::to_string(req_id), "post_id", std::to_string(post_id));
  auto hset_reply_1 = redis_client->hset(std::to_string(req_id), "post_type", std::to_string(post_type));
  auto hlen_reply = redis_client->hincrby(std::to_string(req_id),"num_components", 1);
  redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
  redis_client->sync_commit();
  _redis_client_pool->Push(redis_client_wrapper);
...
void ComposePostHandler::UploadUrls(int64_t req_id, const std::vector<Url> &urls, const std::map<std::string, std::string> &carrier) {
...
void ComposePostHandler::UploadUserMentions(const int64_t req_id, const std::vector<UserMention> &user_mentions, const std::map<std::string, std::string> &carrier) {
  LOG(warning) << "start";
...
void ComposePostHandler::_ComposeAndUpload(int64_t req_id, const std::map<std::string, std::string> &carrier) {
  ...
  post.timestamp = duration_cast<microseconds>(system_clock::now().time_since_epoch()).count();
