...
do_check() {
  ...
  if [ -n "${LOCUST_FILE:+1}" ]; then
  	 ...
  	 LOCUST_FILE="$CRIPTPATH/locustfile.py"
...
#      exit 1
  ...
  locust --host=http://$TARGET_HOST -f $LOCUST_FILE --csv=$CRIPTPATH/example --clients=$CLIENTS --hatch-rate=1 -t $TIME_OUT  --no-web --only-summary
  echo "done"
}
