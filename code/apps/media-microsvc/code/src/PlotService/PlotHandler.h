...
void PlotHandler::ReadPlot(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("ReadPlot", { opentracing::ChildOf(parent_span->get()) });
...
void PlotHandler::WritePlot(
  ...
  if (!plotinsert) {
      LOG(error) << "Error: Failed to insert plot to MongoDB: "<< error.message;
      ServiceException se;
      se.errorCode = ErrorCode::SE_MONGODB_ERROR;
