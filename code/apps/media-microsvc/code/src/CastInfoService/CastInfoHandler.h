...
void CastInfoHandler::WriteCastInfo(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("WriteCastInfo", { opentracing::ChildOf(parent_span->get()) });
  ...
  auto collection = mongoc_client_get_collection(mongodb_client, "cast-info", "cast-info");
  if (!collection) {
      ServiceException se;
      se.errorCode = ErrorCode::SE_MONGODB_ERROR;
  ...
  if (!plotinsert) {
        LOG(error) << "Error: Failed to insert cast-info to MongoDB: "<< error.message;
        ServiceException se;
        se.errorCode = ErrorCode::SE_MONGODB_ERROR;
...
void CastInfoHandler::ReadCastInfo(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("ReadCastInfo", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  if (memcached_rc != MEMCACHED_SUCCESS) {
    LOG(error) << "Cannot get cast_info_ids of request " << req_id << ": "<< memcached_strerror(memcached_client, memcached_rc);
    ServiceException se;
    se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
