...
void ReviewStorageHandler::StoreReview(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("StoreReview", { opentracing::ChildOf(parent_span->get()) });
...
void ReviewStorageHandler::ReadReviews(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("ReadReviews", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  started_ext_call = std::chrono::high_resolution_clock::now();
  memcached_rc = memcached_mget(memcached_client, keys, key_sizes, review_ids.size());
  ended_ext_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_ext_call).count();
  timetaken_ext_call = timetaken_ext_call + ended_ext_call;

  if (memcached_rc != MEMCACHED_SUCCESS) {
        LOG(error) << "Cannot get review-ids of request " << req_id << ": "<< memcached_strerror(memcached_client, memcached_rc);
        ServiceException se;
        se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
  ...
  while (true) {
    ...
    Review new_review;
    json review_json = json::parse(std::string(return_value, return_value + return_value_length));
