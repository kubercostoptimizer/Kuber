...
void ComposeReviewHandler::UploadUniqueId(
  ...
  LOG(debug) << "req_id " << req_id << " caching review_id to Memcached finished";
...
void ComposeReviewHandler::UploadRating(int64_t req_id, int32_t rating, const std::map<std::string, std::string> & carrier) {
  ...
  // Initialize the counter to 0 if there it is not in the memcached
  memcached_rc = memcached_add(memcached_client, key_counter.c_str(), key_counter.size(), "0", 1, MMC_EXP_TIME, 0);
  auto timetaken_mongo_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
  timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;

  // error if it cannot be stored
  if (memcached_rc != MEMCACHED_SUCCESS && memcached_rc != MEMCACHED_DATA_EXISTS) {
    LOG(error) << "Failed to initilize the counter for request " << req_id << " Error code: " << memcached_strerror(memcached_client, memcached_rc);
    ServiceException se;
    se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
    se.message = memcached_strerror(memcached_client, memcached_rc);
    memcached_pool_push(_memcached_client_pool, memcached_client);
  ...
  started_mongo_find = std::chrono::high_resolution_clock::now();
  memcached_rc = memcached_add(memcached_client, key_rating.c_str(), key_rating.size(), rating_str.c_str(), rating_str.size(),MMC_EXP_TIME, 0);
  timetaken_mongo_call = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
  timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;

  if (memcached_rc == MEMCACHED_DATA_EXISTS) {
    // Another thread has uploaded rating, which is an unexpected behaviour.
    LOG(warning) << "rating of request " << req_id << " has already been stored";
    size_t value_size;
    char *counter_value_str = memcached_get(memcached_client, key_counter.c_str(), key_counter.size(), &value_size, 0, &memcached_rc);
    counter_value = std::stoul(counter_value_str);
    free(counter_value_str);
    if (memcached_rc != MEMCACHED_SUCCESS) {
      LOG(error) << "Cannot get the counter of request " << req_id;
