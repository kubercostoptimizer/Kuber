...
void MovieIdHandler::UploadMovieId(
  ...
  char* movie_id_mmc = memcached_get(memcached_client, title.c_str(), title.length(), &movie_id_size, &memcached_flags, &memcached_rc);
  ...
  if (movie_id_mmc) {
        LOG(debug) << "Get movie_id " << movie_id_mmc << " cache hit from Memcached";
...
void MovieIdHandler::RegisterMovieId (
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("RegisterMovieId",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  if (found) {
    ...
    if (!plotinsert) {
        LOG(error) << "Failed to insert movie_id of " << title << " to MongoDB: " << error.message;
        ServiceException se;
        se.errorCode = ErrorCode::SE_MONGODB_ERROR;
