...
class random_ut(unittest.TestCase):
    def setUp(self):
    ...
    def test_run_a1_xlarge(self):
         ...
         self.assertEqual(Random.run(('s1','s2'),results_cont)['name'],'a1.xlarge')
    
    def test_run_M6_large(self):
         results_cont = results_controller.results_controller(False, 100)
         self.assertEqual(Random.run(('s1','s3'),results_cont)['name'],'M6.large')
    
    def test_run_M6_xlarge(self):
        results_cont = results_controller.results_controller(False, 100)
        self.assertEqual(Random.run(('s1','s4'),results_cont)['name'],'M6.xlarge')
