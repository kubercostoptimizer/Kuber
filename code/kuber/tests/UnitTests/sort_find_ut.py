...
class binary_ut(unittest.TestCase):
    ...
    def test_run_a1_xlarge(self):
         ...
         self.assertEqual(sort_find.run(('s1','s2'),results_cont)['name'],'a1.xlarge')
    
    def test_run_M6_large(self):
         results_cont = results_controller.results_controller(False, 100)
         self.assertEqual(sort_find.run(('s1','s3'),results_cont)['name'],'M6.large')
    
    def test_run_M6_xlarge(self):
        results_cont = results_controller.results_controller(False, 100)
        self.assertEqual(sort_find.run(('s1','s4'),results_cont)['name'],'M6.xlarge')
